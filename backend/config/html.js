...
export const getVerifyEmailHtml = ({ email, token }) => {
  ...
  //http://localhost:5173/token/token-created
...
/*
1️⃣ These functions generate HTML email templates for authentication.
2️⃣ getOtpHtml creates an email that shows an OTP for login/verification.
3️⃣ getVerifyEmailHtml creates an email with a verification link (token).
4️⃣ The email looks professional and responsive on all devices.
5️⃣ These HTML strings are sent using Nodemailer to users’ emails.
*/
